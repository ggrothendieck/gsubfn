...
character of \code{\backslash{}1} has string interpolation applied to it
after the \code{\backslash{}1} is removed.}
\item{if there are no formulas with double \code{\~{}} and no
character strings beginning with \code{\backslash{}1} then all formulas
