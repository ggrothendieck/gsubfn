...
the source string.  A second included application of \code{gsubfn}, 
\code{cati}, is like \code{cat} but
provides quasi-perl style string interpolation of
its arguments.   In the case of a replacement
...
it back into the string. \code{cati} is like the \proglang{R} function
\code{cat} except that it uses gsubfn to provide quasi-perl style
...
objects, formulas and associated functions such as \code{strapply}
and \code{cati}.
...
and Section~\ref{sec:cati} explains the use
of \code{cati}.
...
in the four documents listed under Documentation on the \pkg{proto} home page:
\url{http://hhbio.wasser.tu-dresden.de/projects/proto/} .}
...
If the arguments are omitted then the free variables on the
right hand side are used in the order encountered.

\textit{Back References}. 
If the \code{backref} argument is not specified then the match
followed all backreferences are passed to the function as separate
arguments.  If \code{backref} is \code{0} then no back references
are passed.  If \code{backref} is a postive integer, $n$, then the
...
A variation of the last example is available via the command
\code{demo("gsubfn-si")}.
...
\section[Miscellaneous]{\code{"Miscellaneous"}}
\label{sec:cati}
The \code{cati} function is like \code{cat} except it allows
for quasi-perl style string interpolation by prefixing variables
with \code{\$} and enclosing excutable code in backticks.
For example:
...
Also provided are \code{cati0}, \code{cat0} and \code{paste0} which are like 
\code{cati}, \code{cat} and \code{paste} except that the default \code{sep} is 
\code{""}. Here is an example of using paste0.  This example
...
Just preface the function with \code{fn\$} and formula arguments will be intercepted 
...
\code{fn\$} also intercepts the \code{simplify} argument if its value is a function. 
In that case it passes the result through \code{do.call(simplify, result)}. 
For example,
...
The first argument is often intended to be formula.
\code{lm} and lattice \code{xyplot} illustrate this.
Thus
formulas in the first argument are only converted to functions
if they are the only formula among the arguments or if they
have a formal argument name of \code{FUN}.  Formulas among
the second and subsequent arguments are always converted.
That allows the conversion to correctly handle situations like the
following
where the first formula is correctly left untranslated. This
