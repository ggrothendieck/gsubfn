...
Lastly, if \code{fn\$} encounters any character arguments whose first
character is \code{"\\1"} then that argument quasi-perl style string
interpolation is performed on it such that a dollar sign followed by 
a variable is evaluated as that variable and backticked \code{R}
